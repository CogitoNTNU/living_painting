...
def linear_2_oklab(x):
    ...
    li = torch.pow(li, 1.0 / 3.0)
    m = torch.pow(m, 1.0 / 3.0)
    s = torch.pow(s, 1.0 / 3.0)
...
def filter(x):
    ...
    def comp(x, y):
        d = torch.abs(x - y) * 5.0
        return torch.pow(torch.exp(-1.0 * d), 2)
...
def color_match(c, s, o, moment_only=False):
    """
    Constrain the low frequences of the AB channels of output image 'o'
    (containing hue and saturation) to be an affine transformation of 'c' matching the
    mean and covariance of the style image 's'.  Compared to the raw output of
    optimization this is highly constrained, but in practice we find the benefit to
    robustness to be worth the reduced stylization.
    ...
    moment_only -- boolean, prevents applying bilateral filter to AB channels of final
    output to match luminance's edges
    """
