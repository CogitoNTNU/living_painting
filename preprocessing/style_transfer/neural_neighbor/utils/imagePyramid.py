...
def dec_lap_pyr(x, levs):
    ...
    for i in range(levs):
        ...
        h = cur.size(2)
        w = cur.size(3)
        # x_small = F.interpolate( cur, (math.ceil(h / 2), math.ceil(w / 2)),
        #  mode="bilinear")
        x_small = F.interpolate(
            cur, (math.ceil(h / 2), math.ceil(w / 2)), mode="bilinear"
        )
...
def syn_lap_pyr(pyr):
    ...
    for i in range(0, levs - 1)[::-1]:
        # Create new approximation coefficients from current approx. and detail
        # coefficients
